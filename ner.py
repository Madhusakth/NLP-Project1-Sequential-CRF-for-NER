...
if __name__ == '__main__':
    ...
    if system_to_run == "BAD":
        ...
        #dev_decoded = [hmm_model.decode(dev[10].tokens[:5])]
        #dev_decoded = [hmm_model.decode(test_ex.tokens) for test_ex in dev[1]]
        ...
        #dev_decoded = [crf_model.decode(dev[0].tokens)]
        #dev_decoded = [crf_model.decode(test_ex.tokens,idx) for idx, test_ex in enumerate(dev)]
        if args.run_on_test:
            print("Running on test")
            test = read_data(args.blind_test_path)
            test_decoded = [crf_model.decode(test_ex.tokens) for test_ex in test]
            #test_decoded = [crf_model.decode(test[0].tokens)]
            #test_decoded = [crf_model.decode(test_ex.tokens, idx) for idx, test_ex in enumerate(test)]
